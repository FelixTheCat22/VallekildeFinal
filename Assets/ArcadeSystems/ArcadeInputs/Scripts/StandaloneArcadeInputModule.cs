...
namespace UnityEngine.EventSystems
{
    ...
    public class EventMapping
    {
        ...
        public bool useButtonAxisMapping;
        public Vector2 buttonAxisRange;
    }

    [AddComponentMenu("Event/Standalone Arcade Input Module")]
    public class StandaloneArcadeInputModule : PointerInputModule
    {
        /*
        [SerializeField]
        ...
        */
        public int inputModuleID;

        public static List<StandaloneArcadeInputModule> instances = new List<StandaloneArcadeInputModule>();

        public object modalObject = null;

        [SerializeField]
        private EventMapping[] m_HorizontalAxises;
        [SerializeField]
        private EventMapping[] m_VerticalAxises;
        [SerializeField]
        private EventMapping[] m_SubmitButtons;
        [SerializeField]
        private EventMapping[] m_CancelButtons;
        ...
        public static StandaloneArcadeInputModule GetByID (int id)
        {
            return instances.Find(c => c.inputModuleID == id);
        }
        ...
        public EventMapping[] horizontalAxises
        {
            get { return m_HorizontalAxises; }
            set { m_HorizontalAxises = value; }
        }

        /// <summary>
        ///   <para>Input manager name for the vertical axis.</para>
        /// </summary>
        public EventMapping[] verticalAxises
        {
            get
            {
                return this.m_VerticalAxises;
            }
            set
            {
                this.m_VerticalAxises = value;
            }
        }
        ...
        /// </summary>
        public EventMapping[] submitButtons
        {
            get
            {
                return this.m_SubmitButtons;
            }
            set
            {
                this.m_SubmitButtons = value;
            }
        }
        ...
        /// </summary>
        public EventMapping[] cancelButtons
        {
            get
            {
                return this.m_CancelButtons;
            }
            set
            {
                this.m_CancelButtons = value;
            }
        }

        protected StandaloneArcadeInputModule()
        {
        }
        ...
        public override bool ShouldActivateModule()
        {
            ...
            bool flag = this.m_ForceModuleActive | AnyIsDown(this.m_SubmitButtons) | AnyIsDown(this.m_CancelButtons) | !Mathf.Approximately(GetCombinedAxisRaw(this.m_HorizontalAxises), 0.0f) | !Mathf.Approximately(GetCombinedAxisRaw(this.m_VerticalAxises), 0.0f) | (double)(this.m_MousePosition - this.m_LastMousePosition).sqrMagnitude > 0.0 | Input.GetMouseButtonDown(0);
            if (Input.touchCount > 0)
                flag = true;
            return flag;
        }

        bool AnyIsDown (EventMapping[] identifiers)
        {
            for (int i = 0; i < identifiers.Length; i++)
            {
                //Debug.Log(identifiers[i].arcadeInputType + ", player: " + identifiers[i].player + ": Virtual: " + ArcadeInputRun.Instance.GetVirtualMapping(identifiers[i].arcadeInputType, identifiers[i].player));
                //if (Input.GetButtonDown(ArcadeInputRun.Instance.GetVirtualMapping(identifiers[i].arcadeInputType, identifiers[i].player)))
                //{
                //    return true;
                //}

                if (ArcadeInput.InputInitiated(identifiers[i].player, identifiers[i].arcadeInputType, AxisType.Raw, modalObject))
                {
                    return true;
                }
            }
        ...
        float GetCombinedAxisRaw(EventMapping[] identifiers)
        {
            float result = 0;
            for (int i = 0; i < identifiers.Length; i++)
            {
                if (identifiers[i].buttonAxisRange != Vector2.zero)
                {
                    if (ArcadeInput.InputIsActive(identifiers[i].player, identifiers[i].arcadeInputType, AxisType.Raw, modalObject))
                    {
                        //Debug.Log("1");
                        result += identifiers[i].buttonAxisRange.y;
                    } 
                    else
                    {
                        //Debug.Log("2");
                        result += identifiers[i].buttonAxisRange.x;
                    }
                } 
                else
                {
                    
                    result += ArcadeInput.InputAsFullValue(identifiers[i].player, identifiers[i].arcadeInputType, AxisType.Raw, modalObject);
        ...
        protected bool SendSubmitEventToSelectedObject()
        {
            ...
            if (AnyIsDown(this.m_SubmitButtons))
            ...
            if (AnyIsDown(this.m_CancelButtons))
        ...
        private Vector2 GetRawMoveVector()
        {
            ...
            zero.x = GetCombinedAxisRaw(this.m_HorizontalAxises);
            zero.y = GetCombinedAxisRaw(this.m_VerticalAxises);
            if (AnyIsDown(this.m_HorizontalAxises))
            {
            ...
            if (AnyIsDown(this.m_VerticalAxises))
            {
        ...
        protected bool SendMoveEventToSelectedObject()
        {
            ...
            bool flag1 = AnyIsDown(this.m_HorizontalAxises) || AnyIsDown(this.m_VerticalAxises);
